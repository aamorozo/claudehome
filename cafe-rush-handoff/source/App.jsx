...
function GameShell() {
  ...
  // Persistent-ish state (in-memory per session)
  const [money, setMoney] = useState(50);
  const [discovered, setDiscovered] = useState(new Set()); // profession ids
  const [dishesServed, setDishesServed] = useState(new Set()); // dish names
  const [celebsMet, setCelebsMet] = useState(new Set());
  const [levelsCompleted, setLevelsCompleted] = useState(new Set());
  const [ownedUpgrades, setOwnedUpgrades] = useState(new Set());
  const [ownedBackgrounds, setOwnedBackgrounds] = useState(new Set(['cozy']));
  const [activeBackground, setActiveBackground] = useState('cozy');
  ...
  const [dailyCompletedOn, setDailyCompletedOn] = useState(null); // stores seed of last completed day
